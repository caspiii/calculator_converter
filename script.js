...
// Function to handle a new number input after a calculation
function handleNewInput(number) {
    // If the main display is showing a result (after an operation), reset the display
    if (mainDisplay.textContent !== '0' && currentOperator === null && secondOperand === null) {
        mainDisplay.textContent = number;  // Reset the display to the new number
        firstOperand = parseFloat(mainDisplay.textContent);  // Set the new first operand
        ...
        operationHistory.textContent = '';  // Clear the operation history
    } else {
        // Otherwise, append the number as usual
        appendNumber(number);
    }
}
let donenalang = false;
    
// Function to append numbers to the display (existing logic)
function appendNumber(number) {
    if (mainDisplay.textContent === '0' || (currentOperator && secondOperand === null)) {
        mainDisplay.textContent = number;
        console.log ("dnisplay");
    }  else if(donenalang == true && currentOperator == null) {
        clearDisplay();
        mainDisplay.textContent = number;
        donenalang = false;
        console.log("dapat mag reset ka");
    } else {
        mainDisplay.textContent += number;
        console.log("dinisplaykasunod");
    }
...
function toggleSign() {
    ...
    mainDisplay.textContent = -currentValue;  // Toggle the sign on the display

    // Update the operand based on the operator
    if (currentOperator === null) {
        firstOperand = parseFloat(mainDisplay.textContent); // Update first operand
    } else {
        secondOperand = parseFloat(mainDisplay.textContent); // Update second operand
    }
